...
import { BrsFile, TokenUsage } from './BrsFile';
...
describe('BrsFile', () => {
    ...
    describe('tokenChain', () => {
        it('can find a chain of tokens', () => {
            ...
            `);
            program.validate();
            const childFieldToken = file.parser.getTokenAt(Position.create(2, 42));
            const tokenChain = file.getTokenChain(childFieldToken).chain;
            ...
            expect(tokenChain.map(tcm => tcm.usage)).to.eql([TokenUsage.Direct, TokenUsage.DottedGet, TokenUsage.DottedGet]);
        });
        ...
        it('can find a chain of tokens with function call with no args in the middle', () => {
            ...
            `);
            program.validate();
            const childFieldToken = file.parser.getTokenAt(Position.create(2, 49));
            const tokenChain = file.getTokenChain(childFieldToken).chain;
            const tokenChainTokens = tokenChain.map(tcm => tcm.token);
        ...
        it('can find a chain of tokens with function call with multiple args in the middle', () => {
            ...
            `);
            program.validate();
            const childFieldToken = file.parser.getTokenAt(Position.create(2, 75));
            const tokenChain = file.getTokenChain(childFieldToken).chain;
            const tokenChainTokens = tokenChain.map(tcm => tcm.token);
            expect(tokenChain.length).to.equal(4);
            expect(tokenChainTokens.map(token => token.text)).to.eql(['var', 'field', 'funcCall', 'childField']);
        ...
        it('can find a chain of tokens with function call with function call inside', () => {
            ...
            `);
            program.validate();
            const childFieldToken = file.parser.getTokenAt(Position.create(2, 90));
            const tokenChain = file.getTokenChain(childFieldToken).chain;
            const tokenChainTokens = tokenChain.map(tcm => tcm.token);
            expect(tokenChain.length).to.equal(4);
            expect(tokenChainTokens.map(token => token.text)).to.eql(['var', 'field', 'funcCall', 'childField']);
        ...
        it('can find a chain of tokens with array references inside', () => {
            ...
            `);
            program.validate();
            const childFieldToken = file.parser.getTokenAt(Position.create(2, 50));
            const tokenChain = file.getTokenChain(childFieldToken).chain;
            const tokenChainTokens = tokenChain.map(tcm => tcm.token);
            expect(tokenChain.length).to.equal(4);
            expect(tokenChainTokens.map(token => token.text)).to.eql(['var', 'field', 'myArray', 'childField']);
            expect(tokenChain[2].usage).to.eql(TokenUsage.IndexedGet);
        });
        ...
        it('includes unknown when an expression in brackets is part of the chain', () => {
            ...
            `);
            program.validate();
            const toStrToken = file.parser.getTokenAt(Position.create(2, 38));
            const tokenChainResponse = file.getTokenChain(toStrToken);
        ...
        it('includes unknown when an expression in double brackets is part of the chain', () => {
            ...
            `);
            program.validate();
            const toStrToken = file.parser.getTokenAt(Position.create(2, 42));
            const tokenChainResponse = file.getTokenChain(toStrToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.true;
        });
        ...
        it('includes unknown when a complicated expression in brackets is part of the chain', () => {
            ...
            `);
            program.validate();
            const toStrToken = file.parser.getTokenAt(Position.create(2, 81));
            const tokenChainResponse = file.getTokenChain(toStrToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.true;
        });
        it('indicates IndexedGet when referenced via brackets', () => {
            ...
            `);
            program.validate();
            const propAsChainToken = file.parser.getTokenAt(Position.create(3, 48)); // complexObj.subObj.prop
            const propAsAsBracketToken = file.parser.getTokenAt(Position.create(4, 51)); // complexObj["subObj"].prop
            const propAsAsDoubleBracketToken = file.parser.getTokenAt(Position.create(5, 68)); // complexObj["subObj"]["grandChildObj"].prop

            let tokenChainResponse = file.getTokenChain(propAsChainToken);
            ...
            tokenChainResponse = file.getTokenChain(propAsAsBracketToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain[0].usage).to.eql(TokenUsage.IndexedGet);
            tokenChainResponse = file.getTokenChain(propAsAsDoubleBracketToken);
            expect(tokenChainResponse.chain[0].usage).to.equal(TokenUsage.IndexedGet);
        });
        ...
        it('allows token kinds from AllowedLocalIdentifiers as start of a chain', () => {
            ...
            `);
            program.validate();
            const overrideFuncToken = file.parser.getTokenAt(Position.create(2, 40)); // override.someProp.someFunc()
            const stringFuncToken = file.parser.getTokenAt(Position.create(3, 40)); // string.someProp.someFunc()
            const floatFuncToken = file.parser.getTokenAt(Position.create(4, 38)); // float.someProp.someFunc()

            let tokenChainResponse = file.getTokenChain(overrideFuncToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain.map(tcm => tcm.token).map(token => token.text)).to.eql(['override', 'someProp', 'someFunc']);

            tokenChainResponse = file.getTokenChain(stringFuncToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain.map(tcm => tcm.token).map(token => token.text)).to.eql(['string', 'someProp', 'someFunc']);

            tokenChainResponse = file.getTokenChain(floatFuncToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
        ...
        it('allows token kinds from AllowedProperties in middle of a chain', () => {
            ...
            `);
            program.validate();
            const overrideFuncToken = file.parser.getTokenAt(Position.create(2, 40)); // someObj.override.someFunc()
            const stringFuncToken = file.parser.getTokenAt(Position.create(3, 40)); // someObj.string.someFunc()
            const floatFuncToken = file.parser.getTokenAt(Position.create(4, 40)); // someObj.float.someFunc()
        ...
        it('finds tokens in the middle of a chain', () => {
            ...
            const labelToken = file.parser.getTokenAt(Position.create(2, 40)); // 'label'
            const nodesToken = file.parser.getTokenAt(Position.create(2, 31)); // 'nodes'
            const nodesBracketToken = file.parser.getTokenAt(Position.create(2, 34)); // '['
            const bravoToken = file.parser.getTokenAt(Position.create(3, 35)); // 'bravo'
            const otherFuncBracketToken = file.parser.getTokenAt(Position.create(4, 38)); // '('

            let tokenChainResponse = file.getTokenChain(labelToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain.map(tcm => tcm.token).map(token => token.text)).to.eql(['m', 'nodes', 'label']);
            expect(tokenChainResponse.chain.map(tcm => tcm.usage)).to.eql([TokenUsage.Direct, TokenUsage.IndexedGet, TokenUsage.DottedGet]);

            tokenChainResponse = file.getTokenChain(nodesToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain.map(tcm => tcm.token).map(token => token.text)).to.eql(['m', 'nodes']);
            expect(tokenChainResponse.chain.map(tcm => tcm.usage)).to.eql([TokenUsage.Direct, TokenUsage.DottedGet]);

            tokenChainResponse = file.getTokenChain(nodesBracketToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain.map(tcm => tcm.token).map(token => token.text)).to.eql(['m', 'nodes']);
            expect(tokenChainResponse.chain.map(tcm => tcm.usage)).to.eql([TokenUsage.Direct, TokenUsage.IndexedGet]);

            tokenChainResponse = file.getTokenChain(bravoToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain.map(tcm => tcm.token).map(token => token.text)).to.eql(['alpha', 'bravo']);
            expect(tokenChainResponse.chain.map(tcm => tcm.usage)).to.eql([TokenUsage.Direct, TokenUsage.DottedGet]);

            tokenChainResponse = file.getTokenChain(otherFuncBracketToken);
            expect(tokenChainResponse.includesUnknowableTokenType).to.be.false;
            expect(tokenChainResponse.chain.map(tcm => tcm.token).map(token => token.text)).to.eql(['m', 'otherFunc']);
