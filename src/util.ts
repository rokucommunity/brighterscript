...
export class Util {
    ...
    public findBeginningVariableExpression(dottedGet: DottedGetExpression): VariableExpression | undefined {
        let left: AstNode | undefined = dottedGet;
        while (left) {
    ...
    public safeJsonStringify(data: any, replacer?: (key: string, value: any) => any, space?: string | number) {
        ...
        const visit = (obj: any) => {
            ...
            try {
                ...
                return Object.keys(obj as Record<string, unknown>).reduce((result, prop) => {
    ...
    public validateTooDeepFile(file: (BrsFile | XmlFile)) {
        ...
        let pkgPath: string = file.pkgPath;
...
export function standardizePath(stringParts, ...expressions: any[]) {
    ...
    for (let i = 0; i < stringParts?.length; i++) {
        result.push(stringParts[i] as string, expressions[i] as string);
    }
