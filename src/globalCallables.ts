...
import { Parser, FunctionStatement, FunctionExpression, FunctionParameter, Block, VariableExpression, LiteralExpression } from './parser';
import { Identifier, negativeRange, Token } from './lexer/Token';
...
let mathFunctions: ShorthandCallable[] = [{
    name: 'Abs',
...
let runtimeFunctions: ShorthandCallable[] = [{
    name: 'CreateObject',
...
let globalUtilityFunctions: ShorthandCallable[] = [
    {
        name: 'Sleep',
...
let globalStringFunctions: ShorthandCallable[] = [
    {
        name: 'UCase',
...
let programStatementFunctions: ShorthandCallable[] = [
    {
        name: 'Tab',
...
const asToken = token(TokenKind.As, 'as');
const equalsToken = token(TokenKind.Equal, '=');
const block = new Block([], negativeRange);
const endToken = token(TokenKind.EndFunction, 'end function');
const leftParen = token(TokenKind.LeftParen, '(');
const rightParen = token(TokenKind.RightParen, ')');
const defaultValue = new LiteralExpression(token(TokenKind.Integer, '1'));
export let globalCallables = [
    ...mathFunctions,
    ...runtimeFunctions,
    ...globalUtilityFunctions,
    ...globalStringFunctions,
    ...programStatementFunctions
].map((x) => {
    const func = new FunctionExpression(
        x.params.map(p => new FunctionParameter(
            identifier(p.name),
            p.isOptional === false ? equalsToken : null,
            //just pass any default value in, these shouldn't get validated...
            p.isOptional === false ? null : defaultValue,
            asToken,
            token(util.typeToTokenKind(p.type))
        )),
        block,
        ...
        rightParen,
        asToken,
        token(util.typeToTokenKind(x.type))
    );

    let callable = new FunctionStatement(identifier(x.name), func, null) as Callable;
    callable.func.typeRef.links = [
        x.type
    ];
    callable.file = globalFile;
    return callable;
});

globalFile.callables = globalCallables;

function token(kind: TokenKind, text?: string) {
    return {
        isReserved: false,
        kind: kind,
        range: negativeRange,
...
function identifier(text?: string) {
    return token(TokenKind.Identifier, text) as Identifier;
}

/**
 * A map of all built-in function names. We use this extensively in scope validation
 * so keep a single copy in memory to reduce memory usage
 */
export const globalCallableMap = globalCallables.reduce((map, x) => {
    map[x.name.text.toLowerCase()] = x;
    return map;
}, {});

interface ShorthandCallable {
    name: string;
