...
describe.only('Program', () => {
    ...
    describe('getCompletions', () => {
        ...
        it('get all functions and properties relevant for m ', async () => {
            ...
            expect(
                (await program.getCompletions(`${rootDir}/source/main.bs`, Position.create(4, 30))).map(x => x.label).sort()
            ).to.eql(['personAMethodA', 'personAMethodB', 'personAMethodC', 'personAName', 'personBMethodA', 'personBMethodB', 'personBName', 'personCMethodA', 'personCMethodB', 'personCMethodC', 'personCName', 'personName']);
            expect(
                (await program.getCompletions(`${rootDir}/source/main.bs`, Position.create(25, 26))).map(x => x.label).sort()
    ...
    describe('getSignatureHelp', () => {
        it('gets signature help for class function with params', async () => {
            await program.addOrReplaceFile('source/main.bs', `
            function main()
                p = new Person()
                p.sayHello()
            ...
            `);
            let signatureHelp = (await program.getSignatureHelp(`${rootDir}/source/main.bs`, 'sayHello', '.', 'p'));
            expect(program.getDiagnostics()[0]?.message).not.to.exist;
            expect(signatureHelp[0].label).to.equal('function sayHello()');
        });

        it('gets signature help for class function', async () => {
            await program.addOrReplaceFile('source/main.bs', `
            function main()
            end function
            ...
            let signatureHelp = (await program.getSignatureHelp(`${rootDir}/source/main.bs`, 'sayHello', '.', 'p'));
            expect(program.getDiagnostics()[0]?.message).not.to.exist;
            expect(signatureHelp[0].label).to.equal('function sayHello(text)');
        });
        ...
        it('gets signature help for namespace function', async () => {
            ...
            `);
            let signatureHelp = (await program.getSignatureHelp(`${rootDir}/source/main.bs`, 'sayHello', '.', 'person'));
            expect(program.getDiagnostics()[0]?.message).not.to.exist;
            expect(signatureHelp[0].label).to.equal('function sayHello(text, text2)');
        });
        ...
        it('gets signature help for nested namespace function', async () => {
            ...
            namespace person.george
                function sayHello(text, text2)
                end function
            end namespace
            `);
            let signatureHelp = (await program.getSignatureHelp(`${rootDir}/source/main.bs`, 'sayHello', '.', 'person.george'));
            expect(program.getDiagnostics()[0]?.message).not.to.exist;
            expect(signatureHelp[0].label).to.equal('function sayHello(text, text2)');
        });
        ...
        it('gets signature help for callfunc method', async () => {
            ...
            let signatureHelp = (await program.getSignatureHelp(`${rootDir}/source/main.bs`, 'sayHello', '@.', 'myNode'));
            expect(program.getDiagnostics()[0]?.message).not.to.exist;
